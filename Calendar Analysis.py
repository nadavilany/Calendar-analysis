...
def category_sort(subject, category):  # inside and outside sorting
    try:
        if category == "Outside meeting": #here you can add more categories that you want to include as outside meeting
            out = "Outside meeting"
        elif category == "Private": #deleting the private meetings
            out = "Delete"
        else:
            out = "Manual sorting needed" #couldnt sort by category
        return out
    except TypeError:
        return "Delete"
...
def main():
    ...
    try:
        ...
        df["Category sort"] = df.apply(lambda row: category_sort(row["Subject"], row["Categories"]),
